...
s.PatientName = par_key(ch, 'Patient name', 0);
s.StudyDescription = par_key(ch, 'Examination name', 0);
s.SeriesDescription = nam;
s.ProtocolName = par_key(ch, 'Protocol name', 0);
a = par_key(ch, 'Examination date/time', 0);
a = a(isstrprop(a, 'digit'));
s.AcquisitionDateTime = a;
s.SeriesNumber = par_key(ch, 'Acquisition nr');
% s.ReconstructionNumberMR = par_key(ch, 'Reconstruction nr');
% s.MRSeriesScanDuration = par_key(ch, 'Scan Duration');
s.NumberOfEchoes = par_key(ch, 'Max. number of echoes');
nSL = par_key(ch, 'Max. number of slices/locations');
s.LocationsInAcquisition = nSL;
a = par_key(ch, 'Patient position', 0);
if isempty(a), a = par_key(ch, 'Patient Position', 0); end
if ~isempty(a)
    if numel(a)>4, s.PatientPosition = a(regexp(a, '\<.')); 
    else, s.PatientPosition = a; 
    end
end
s.MRAcquisitionType = par_key(ch, 'Scan mode', 0);
s.ScanningSequence = par_key(ch, 'Technique', 0); % ScanningTechnique
typ = par_key(ch, 'Series Type', 0); typ(isspace(typ)) = '';
s.ImageType = ['PhilipsPAR\' typ '\' s.ScanningSequence];
s.RepetitionTime = par_key(ch, 'Repetition time');
s.WaterFatShift = par_key(ch, 'Water Fat shift');

rotAngle = par_key(ch, 'Angulation midslice'); % (ap,fh,rl) deg
rotAngle = rotAngle([3 1 2]); % this seems always
posMid = par_key(ch, 'Off Centre midslice'); % (ap,fh,rl) [mm]
posMid = posMid([3 1 2]); % better precision than those in the table
s.EPIFactor = par_key(ch, 'EPI factor');
% s.DynamicSeries = par_key(ch, 'Dynamic scan'); % 0 or 1
isDTI = par_key(ch, 'Diffusion')>0;
if isDTI
    s.ImageType = [s.ImageType '\DIFFUSION\'];
    s.DiffusionEchoTime = par_key(ch, 'Diffusion echo time'); % ms
end
...
if strncmp(V, 'V3', 2)
    s.BitsAllocated = par_key(ch, 'Image pixel size', 1);
    s.Columns = par_key(ch, 'Recon resolution', 1);
    s.SliceThickness = par_key(ch, 'Slice thickness', 1);
    s.SpacingBetweenSlices = par_key(ch, 'Slice gap', 1);
else
...
a = par_key(ch, 'Preparation direction', 0); % Anterior-Posterior
if ~isempty(a)
    a = a(regexp(a, '\<.')); % 'AP'
...
s.PixelData.Bytes = s.Rows * s.Columns * nFrame * s.BitsAllocated / 8;
    ...
    function val = par_key(ch, key, isNum)
        expr = ['\n.\s*' key '.*?:(.*?)\n']; % \n. key ... : val \n
...
if ~isequal(a, 1:nFrame), error('Unusual REC Index not supported'); end
...
% Rotation order and signs are figured out by try and err, not 100% sure
a = [xml_attr(ch1, 'Angulation RL', 1) xml_attr(ch1, 'Angulation AP', 1) ...
     xml_attr(ch1, 'Angulation FH', 1)] /180*pi; % radians
R = makehgtform('xrotate', a(1), 'yrotate', a(2), 'zrotate', a(3));
...
s.ImageOrientationPatient = R([1:3 5:7])';
R = R * diag([s.PixelSpacing; s.SpacingBetweenSlices; 1]);
R(1:3, 4) = [xml_attr(ch1, 'Off Center RL', 1)
